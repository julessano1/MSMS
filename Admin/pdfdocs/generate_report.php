...
//Page footer
    function Footer() {
        ...
        $this->Cell(0, 37, '@ Finencial Management System . All Rights Reserved.', 0, 0, 'C');
...
//BUDGES
      ...
      $pdf->setDrawColor(0,0,0);// Set color for all lines
		...
		$pdf->line($x-6,$y+250,$x+160,$y+250);// hrizontal line 5
		 ...
		 $pdf->Image('../../img/favicon.jpg',$x+140,$y,25);
	...
	$pdf->SetTextColor(0,0,0);
		...
		$pdf->Text($x+25,$y+10,"KIGALI CITY");
		$pdf->Text($x+25,$y+15,"nyarugenge DISTRICT");
		$pdf->Text($x+25,$y+20,"NYARUGENGE SECTOR");
		$pdf->Text($x+25,$y+25,"FINENCIAL MANAGEMENT SYSTEM");
		$pdf->Text($x+25,$y+30,"Email: finencislms@gmail.com");
		$pdf->Text($x+25,$y+35,"Tel: 0788 888 888 / 0788 666 888 / 200 313 268");
...
$date2=date('d-m-Y H:i:s');
		...
		$pdf->Text($x+30,$y+58,'Contribution Report '.$report_type);
		...
		$pdf->Text($x+25,$y+67,'Member Name');
		$pdf->Text($x+62,$y+67,'Member ID No ');
//		$pdf->Text($x+102,$y+67,'Member Phone ');
//		
		$pdf->Text($x+99,$y+67,' Contribution ');
        $pdf->Text($x+126,$y+67,'Contribution Date');
		...
		// Load the products
		 ...
		 $stms_icome = $conn->query("SELECT contribution.Share_Amount,contribution.MemberName,contribution.MemberID_No,contribution.MemberAddress,contribution.Date,members.Member_Phone FROM contribution INNER JOIN members ON contribution.MemberID=members.MemberID GROUP BY contribution.MemberName ");
               echo $conn->error;
...
//								$paymentInID = $rows['paymentInID'];
//								$PaymentAccountName = $rows['PaymentAccountName'];								
//								$paymentAmount = $rows['paymentAmount'];
//								$currencyCode = $rows['currencyCode'];
//								$paymentMethodName = $rows['paymentMethodName'];
//								$referenceNumber = $rows['referenceNumber'];
//								$Incom_date = $rows['Incom_date'];
//								$user_name = $rows['empFamilyName'];
                                
								
//	            $Loan_ApplicationID = $rows['Loan_ApplicationID'];
                $TotalContribution=$rows['Share_Amount'];
                $MemberName=$rows['MemberName'];
                $MemberID_No=$rows['MemberID_No'];
                $MemberPhone=$rows['Member_Phone'];
                $Member_Address=$rows['MemberAddress'];
                $ContributionDate=$rows['Date'];							
	$pdf->SetFont('Arial','',11);
	$pdf->Text($x,$y+$yy,$i);// Display the order
	$pdf->Text($x+$xx,$y+$yy,$MemberName);// Display the product list
	$pdf->Text($x+51+$xx,$y+$yy,$MemberID_No);// Display Buying price
    $pdf->Text($x+92+$xx,$y+$yy,$TotalContribution." "."Rwf");// Display 
	$pdf->SetFont('Arial','B',8);
	$pdf->SetTextColor(255,0,0);
	//$pdf->Text($x+125+$xx,$y+$yy+4,'Discount:'.$referenceNumber);// Discount
	$pdf->SetFont('Arial','',11);
	$pdf->SetTextColor(0,0,0);
	$pdf->Text($x+120+$xx,$y+$yy,$ContributionDate);// Display price
